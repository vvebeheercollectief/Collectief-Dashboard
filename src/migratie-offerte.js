...
export async function migreerOfferteStappen(){
  ...
  // en die stand is pas terug te draaien door een verse ronde.
  ...
  const { naarOpvolg, zonderSub } = migratieSelectie(rows, bouwBundelIndex(D.ntd, D.af));
  ...
  if(data.length){
    // `serieleWrite` en niet `backgroundWrite`: die laatste eist een rollback-functie en levert
    // toasts, en dit is een handmatige eenmalige routine waarvan de uitslag in de console hoort.
    // GEEN rollback dus — bewust. Faalt deze ene batch, dan is er niets geschreven (één atomaire
    // values:batchUpdate, alles-of-niets), blijft de optimistische stand op het scherm staan tot
    // de eerstvolgende poll hem overschrijft, en levert een tweede run exact dezelfde selectie op.
    // De idempotente selectie IS hier het vangnet.
    //
    ...
    await serieleWrite(async()=>{
      ...
      if(!resp.ok){
        // .catch op het foutlichaam: een 502 van een tussenliggende proxy stuurt HTML terug, en dan
        // zou resp.json() een SyntaxError gooien die als netwerkfout telt (zelfde reden als in
        // writeRange en schrijfBereiken). Bij een 401 het token wissen, anders blijft elke
