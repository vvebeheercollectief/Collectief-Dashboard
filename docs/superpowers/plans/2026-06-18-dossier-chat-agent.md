...
- [ ] **Step 3: Backend-eenmalig (beheerder, met begeleiding)** — in het **Vercel-dashboard** → Project → Settings → Environment Variables: `ANTHROPIC_API_KEY` = de sleutel uit de Anthropic Console, voor Production én Preview. In de **Anthropic Console**: een spend limit op de sleutel zetten. (Vercel deployt `api/chat.js` automatisch met de push mee.)
